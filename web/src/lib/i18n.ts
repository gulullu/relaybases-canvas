...
const zhToEn = {
    ...
    "用于图片和视频生成。建议在主站用 gpt-image-2 分组创建媒体 Key；异步图片和异步视频任务按 4x 计费。": "Used for image and video generation. Create the media key from the gpt-image-2 group on RelayBases. Async image and async video tasks are billed at 4x.",
    ...
    视频模型: "Video Models",
    "默认同步调用；用户也可以切换为异步任务，异步按 4x 计费。": "Uses sync calls by default. Users can switch to async tasks, which are billed at 4x.",
    "生图默认使用同步接口；视频默认同步调用，按需可切换异步 4x；文本模型用于 Agent 和文本节点。": "Image generation defaults to sync endpoints. Video defaults to sync calls and can be switched to async 4x. Text models power agents and text nodes.",
