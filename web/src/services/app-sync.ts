    ...
    return syncAppDataToRemote(
        {
            ...
            writeFile: (path, file, contentType) => uploadWebdavFile(config, path, file, contentType),
        },
        onProgress,
    );
}
...
async function syncDomain<T>(storage: RemoteSyncStorage, onProgress: AppSyncProgress | undefined, options: SyncDomainOptions<T>): Promise<SyncDomainResult<T>> {
    try {
        ...
        await storage.writeFile(domainPath(options.key, WEBDAV_MANIFEST_FILE_NAME), manifestFile, "application/json");
...
async function readDomainManifest<T>(storage: RemoteSyncStorage, domain: DomainKey, emptyData: T): Promise<DomainManifest<T> | null> {
    const file = await storage.readFile(domainPath(domain, WEBDAV_MANIFEST_FILE_NAME));
    if (!file) return null;
...
async function downloadMissingFiles<T>(storage: RemoteSyncStorage, domain: DomainKey, data: T, remoteFiles: AppSyncFile[], onProgress?: AppSyncProgress) {
    ...
    await runWithConcurrency(tasks, FILE_CONCURRENCY, async (remoteFile) => {
        const blob = await storage.readFile(remoteFile.path);
        if (!blob) return;
...
async function uploadChangedFiles<T>(storage: RemoteSyncStorage, domain: DomainKey, data: T, remoteFiles: AppSyncFile[], onProgress?: AppSyncProgress) {
    const remoteFileMap = new Map(remoteFiles.map((item) => [item.storageKey, item]));
    ...
    await runWithConcurrency(tasks, FILE_CONCURRENCY, async ({ item, blob }) => {
        await storage.writeFile(item.path, blob, item.mimeType);
        uploadedFiles += 1;
